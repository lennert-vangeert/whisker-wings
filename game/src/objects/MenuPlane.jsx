...
const MenuPlane = () => {
  ...
  useFrame(({ clock, camera }) => {
    if (planeRef.current) {
      ...
      planeRef.current.rotation.z = Math.sin(elapsedTime * rotationSpeed) * 0.05
      planeRef.current.rotation.x = Math.cos(elapsedTime * rotationSpeed) * 0.02
    }
    // set camera position
  ...
  useEffect(() => {
    if (planeRef.current) {
      ...
      // Screen-space note: the camera sits at (-203, 115.5, 271) looking at
      // (-198, 113, 265), which works out to a camera-right of ~(0.732, 0, 0.610).
      // So pure screen-left is ~(-0.732, 0, -0.610) — moving x alone drifts the
      // plane away from the camera as well as sideways.
      //
      // The panel pages get a proper screen-left push so the plane clears the
      // right-hand panel entirely.
      if (
        menuPhase === 'leaderboards' ||
        menuPhase === 'credits' ||
        menuPhase === 'controls'
      ) {
        targetPosition = [-199.5, 113, 263.2]
